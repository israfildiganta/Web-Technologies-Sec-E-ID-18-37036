...
<?php
   ...
   if($_SERVER["REQUEST_METHOD"] == "POST") 
   {
       ...
       else 
       {
          ...
          {
            $_SESSION['login']=1;
            setcookie('customer_name',$res['customer_name'],time()+60);
            $_SESSION['customer_name']=$res['customer_name'];

            setcookie('customer_email',$res['customer_email'],time()+60);
            $_SESSION['customer_email'] = $res["customer_email"];


            setcookie('customer_phone',$res['customer_phone'],time()+60);
            $_SESSION['customer_phone'] = $res["customer_phone"];

            setcookie('customer_user_name',$res['customer_user_name'],time()+60);
            $_SESSION['customer_user_name'] =$res["customer_user_name"];

            setcookie('current_password',$res['current_password'],time()+60);
            $_SESSION['current_password'] = $res["current_password"];

            setcookie('customer_gender',$res['customer_gender'],time()+60);
            $_SESSION['customer_gender'] = $res["customer_gender"];

            setcookie('customer_bg',$res['customer_bg'],time()+60);
            $_SESSION['customer_bg'] = $res["customer_bg"];

            setcookie('customer_DOB',$res['customer_DOB'],time()+60);
            $_SESSION['customer_DOB'] = $res["customer_DOB"];

            setcookie('customer_id',$res['customer_id'],time()+60);
